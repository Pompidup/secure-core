...
When `recovery::unwrap_dek_with_passphrase` is asked to unwrap a `RecoveryWrap`
whose `schema_version` is not the version supported by the current build, the
function SHALL return a structured error case that is distinct from the generic
invalid-parameter case used for malformed input (bad base64, wrong IV/tag
length, empty passphrase). The structured error SHALL carry the offending
`schema_version` value for diagnostics.
